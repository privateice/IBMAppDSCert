...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                dcc.Dropdown(id='site_dropdown', options=[{'label': 'All Sites', 'value': 'ALL'}, {'label': 'CCAFS LC-40', 'value': 'CCAFS LC-40'}, {'label': 'CCAFS SLC-40', 'value': 'CCAFS SLC-40'}, {'label': 'KSC LC-39A', 'value': 'KSC LC-39A'}, {'label': 'VAFB SLC-4E', 'value': 'VAFB SLC-4E'}], value='ALL', placeholder="Select a launch site...", searchable=True),
...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        # Create a new column 'Outcome' with 'Success' and 'Failure' labels
        filtered_df['Outcome'] = filtered_df['class'].map({0: 'Failure', 1: 'Success'})

        # Render and return a pie chart graph to show success (class=1) and failed (class=0) count for the selected site
        fig = px.pie(filtered_df, names='Outcome', 
                     title=f'Success and Failure Counts for {entered_site}',
                     labels={'Failure': 'Failures', 'Success': 'Successes'})
    return fig
...
def update_scatter_chart(entered_site, selected_payload_range):
    ...
    fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class', color='Booster Version Category',
                     title='Payload vs. Launch Outcome with Booster Version Category')
    return fig
